...
impl Templates {
    pub fn load() -> Self {
        ...
        from_reader(file).expect("Unable to oad templates")
    }
    ...
    pub fn spawn_entities(
        ...
        let mut available_enteties = Vec::new();
        self.entities
            .iter()
            .filter(|e| e.levels.contains(&level))
            .for_each(|t| {
                for _ in 0 .. t.frequency {
                    available_enteties.push(t);
                }
            });

        let mut commands = CommandBuffer::new(&ecs);
        spawn_points.iter().for_each(|pt| {
            if let Some(entity) = rng.random_slice_entry(&available_enteties) {
